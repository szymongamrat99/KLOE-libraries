...
void neu_triangle(Float_t *TrcSumFinal, Float_t *vtxSigmaFinal, Float_t Clu5Vec[4][5], Float_t *ip, Float_t *Phi4Mom, Float_t *Kne4Mom, Float_t *Kne4Vec, Float_t *trc)
{
  // Logging of errors
  ofstream LogFile;
  ...
  for (Int_t i = 0; i < 4; i++)
  {
    ...
    B[i] = 2 * (DTot[i] * CosPkD[i] - (c_vel * Clu5Vec[i][3] / BetaK));
    C[i] = pow(c_vel * Clu5Vec[i][3], 2) - pow(DTot[i], 2);

    Delta[i] = pow(B[i], 2) - 4 * A * C[i];

    if (Delta[i] < 0.)
    {
      LogFile << "Cluster " << i << ": Delta less than 0! Breaking." << endl;
      break;
    }
    else if (A == 0.)
    {
      LogFile << "Cluster " << i << ": A equal to 0! Breaking." << endl;
      break;
    }
    else
    {
  ...
  for (Int_t i = 0; i < 4; i++)
  {
    ...
    trc[i] = Clu5Vec[i][3] - NeuVtxAvg[3] - (lGammaFinal[i] / c_vel);

    vtxSigma += Clu5Vec[i][4] * sqrt(pow(NeuVtxTrueClu[i][0] - NeuVtxAvg[0],2) +
                                     pow(NeuVtxTrueClu[i][1] - NeuVtxAvg[1],2) +
                                     pow(NeuVtxTrueClu[i][2] - NeuVtxAvg[2],2) ) / EneTot;
