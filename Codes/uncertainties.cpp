...
Double_t clu_x_error(Float_t z_coor, Float_t Enecl)
{
    Double_t sigma_x = 0.;

    if ( abs(z_coor) > 165. )
        sigma_x = sigma_coor / sqrt( 0.001*Enecl );
    else
        sigma_x = sigma_coor;
}

Double_t clu_y_error(Float_t z_coor, Float_t Enecl)
{
    Double_t sigma_y = 0.;
...
Double_t clu_z_error(Float_t z_coor, Float_t Enecl)
{
    Double_t sigma_z = 0.;
